...
function buildPrompt(user) {
    ...
    const creditLabel = user.creditScore >= 750 ? 'Excellent' :
...
const parseLogger = new common_1.Logger('AiRecommendationsParser');
function parseAiResponse(raw) {
    ...
    if (!Array.isArray(parsed) || parsed.length === 0) {
        ...
        throw new common_1.BadRequestException('AI returned an empty or invalid recommendations list.');
    }
...
let AiRecommendationsService = AiRecommendationsService_1 = class AiRecommendationsService {
    prisma;
    logger = new common_1.Logger(AiRecommendationsService_1.name);
    constructor(prisma) {
        this.prisma = prisma;
    }
    async generate(dto) {
        ...
        if (dto.userId) {
            ...
            if (!user.netSalary || !user.creditScore) {
                throw new common_1.BadRequestException('User must complete the questionnaire before generating AI recommendations.');
            }
            profile = {
            ...
            if (!dto.netSalary || !dto.creditScore) {
                throw new common_1.BadRequestException('netSalary and creditScore are required for guest recommendations.');
            }
            profile = {
...
["used cars for sale Gauteng under R200000 site:autotrader.co.za", "new cars for sale Gauteng under R220000 site:autotrader.co.za", "..."]`;
            ...
            const searchResults = await Promise.allSettled(queries.map((q) => this.serperSearch(q, serperKey)));
            const snippets = [];
...
Return ONLY a raw JSON array — no markdown, no explanation.`;
        ...
        if (!raw)
            throw new common_1.BadRequestException('OpenAI returned an empty response.');
    ...
    async searchWithAnthropic(prompt) {
    ...
    async serperImageSearch(query, apiKey) {
        try {
    ...
    async attachImages(recs, serperKey) {
        const withImages = await Promise.all(recs.map(async (rec) => {
...
Return ONLY a raw JSON array — no markdown, no explanation.`;
        ...
        if (!raw) {
            throw new common_1.BadRequestException('Gemini returned an empty response. Please try again.');
        }
