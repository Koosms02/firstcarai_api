...
const TOOLS = [
    {
        ...
        function: {
            ...
            description: 'Update the user\'s monthly expense amounts to help reduce spending and improve their car affordability. Only call this when the user asks to cut costs or update expenses.',
            parameters: {
                type: 'object',
                properties: {
    ...
    },
    {
        ...
        function: {
            name: 'update_profile',
            description: 'Update the user\'s profile details such as their salary or province. Only call when the user explicitly provides new values.',
            parameters: {
                type: 'object',
                properties: {
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Trigger a fresh car search based on the user\'s current or updated budget and preferences. Call this when the user asks to find cars, see what they can afford, or after updating their finances.',
            parameters: {
                type: 'object',
                properties: {
...
- Keep responses short — 2–4 paragraphs unless asked for more detail.`;
        ...
        const toolMessages = [];
        toolMessages.push(firstChoice.message);
        for (const toolCall of firstChoice.message.tool_calls) {
            ...
            if (toolCall.function.name === 'update_expenses') {
                ...
                actions.push(action);
                const newTotal = [
                    args.groceries ?? fc.expenses.groceries,
                    args.accounts ?? fc.expenses.accounts,
                    args.loans ?? fc.expenses.loans,
                    args.other ?? fc.expenses.other,
                ].reduce((a, b) => a + b, 0);
                ...
                const action = {
                    ...
                    yearsLicensed: args.yearsLicensed,
                };
                actions.push(action);
                const salary = args.netSalary ?? fc.netSalary;
                ...
                actions.push(action);
                toolResult = `Car search triggered with a budget of R ${args.budget.toLocaleString('en-ZA')} per month. Results will appear on screen.`;
            }
            toolMessages.push({
                role: 'tool',
